...
def compute_pareto_frontier(df, x_col, y_col, threshold_x=0.025, threshold_y_ratio=4):
    """
...
def plot_5_2_1_comparison(all_data):
    ...
    for query in query_set:
        legend_elements.append(plt.Line2D([0], [0], color=algorithm_colors[query], marker=algorithm_markers[query], linestyle=algorithm_line_styles[query],
                                            label=selectivity_mapping[query], **plot_legend_params))
...
# 创建一个新的函数用于绘制多线程算法图表
def plot_5_2_2_comparison(all_data):
...
if not all_data:
    ...
    plt.savefig(save_path + "exp_5_2_1.svg", bbox_inches='tight')
    plt.savefig(save_path + "exp_5_2_1.pdf", bbox_inches='tight')
    plt.close(fig1)
