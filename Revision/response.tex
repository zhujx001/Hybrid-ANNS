...
\textbf{WIT-Real dataset:} We use the \texttt{clip-vit-base-patch32} model to encode the \texttt{page\_title} field from the WIT dataset into vectors, and extract the language type and the length of the \texttt{context\_page\_}
\texttt{description} field as real attributes, applying them in both attribute filtering and range filtering experiments.
...
We did observe slightly lower QPS on this server compared to our original one, which is mainly due to the lower CPU frequency of the Xeon 6330 processors. \textbf{(Please refer to the blue highlighted parts in Section 4.2.4 and Section 4.3.4 under Different Hardware Environments.)}
...
\noindent
\textbf{Response:} I really appreciate your suggestion. Current hybrid search algorithms don't explicitly model or leverage complex semantic relationships, such as those between vectors and their attributes, or among different attributes. However, our evaluation covered a range of realistic scenarios, including multi-attribute setting, diverse distributions and selectivity, to assess the algorithms' generalization capabilities (though we used synthetic attributes).

To demonstrate that the choice between synthetic and real attributes has little impact on algorithm performance, we conducted an additional set of experiments. We used the YouTube-Audio dataset for vectors and compared results with both synthetic and real attributes (video classification). The experimental outcomes validated our hypothesis.
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have added relevant experiments conducted on different servers in Section 4.2.4. Please refer to the response MR.R4 for more details.
%%%%%%%%%%%%%%%%%%%%%% W3 %%%%%%%%%%%%%%%%%%%%%%
