...
\noindent
\textbf{Response:} 
Thank you for your suggestion. Using ID as attribute for range filtering is a prevalent method in current range filtering algorithm evaluations, which indeed has considerable limitations. To address this issue, we have added two datasets that use real attributes.
...
We did observe slightly lower QPS on this server compared to our original one, which is mainly due to the the lower CPU frequency, reduced RAM capacity, and smaller L3 cache of the Xeon 6330 processors. \textbf{(Please refer to the blue highlighted parts in Section 4.2.4 and Section 4.3.4 under Different Hardware Environments.)}
