...
\noindent
\textbf{Response:} 
Thank you for your valuable suggestion regarding our experiments. In Section 4.2.3 and Section 4.3.3, we have added experiments on the Deep100M dataset. Due to insufficient memory on our original server for running algorithms on such large-scale datasets, we have conducted these experiments on a server with larger memory capacity, equipped with 2×AMD 7Y43 processors and 1TB of RAM. The results show that most algorithms struggle to balance performance with time and space costs on this dataset. In particular, StitchedVamana performs poorly on Deep100M. The experimental results are shown in Figure~\ref{fig:attribute big dataset} and Figure~\ref{fig:range big dataset}.
...
Section 4.1.3:\textcolor{blue}{"We test on  large-scale dataset using a server with 2×AMD 7Y43 processors and 1TB RAM."}
