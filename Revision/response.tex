...
\textcolor{blue}{"The proximity of the curves in the figure reflects algorithm sensitivity to distributional variations. When the curves converge closely, the algorithm demonstrates strong resilience against attribute distribution shifts."}
...
We also removed the original Figure~\ref{fig:attribute_selectivity_2}, which displayed too many algorithms in one plot and was difficult to interpret and somewhat redundant with Figure~\ref{fig:attribute_selectivity}. We believe these changes improve the clarity and readability of the paper, without reducing the completeness of the evaluation.
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. Using the ID as an attribute for range filtering is a prevalent method in current range filtering algorithm evaluations, which indeed has considerable limitations. To address this issue, we have updated two of our datasets (YT-Audio-Real, WIT-Real) to include real attributes.In fact, current hybrid query algorithms do not explicitly model or leverage complex semantic relationships, such as those between vectors and their attributes, or among different attributes. As shown in Figure~\ref{fig:range_2_2}, our experiments also show that the impact of using real attributes versus using IDs on the algorithms is negligible. The key factor remains the size of the query range. \textbf{(Please see the third paragraph of Section 4.1.1, highlighted in blue in the revised manuscript.)}
