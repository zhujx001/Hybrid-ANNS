...
\begin{figure}[t]
	...
	% 直接放置第二张图片，可以加一点垂直间距
	\vspace{5pt} 
	\includegraphics[width=0.99\columnwidth]{fig/range_2_2.pdf}
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. Using the ID as an attribute for range filtering is a prevalent method in current range filtering algorithm evaluations, which indeed has considerable limitations. To address this issue, we have changed two datasets (YT-Audio-Real, WIT-Real) to use real attributes. We observe that the impact of using real attribute versus using ID on the algorithms is negligible; the key factor remains the size of the query range, as shown in Figure~\ref{fig:range_2_2}. \textbf{(Please see the third paragraph of Section 4.1.1, highlighted in blue in the revised manuscript.)}
...
\noindent
\textbf{Response:} 
Thank you for your feedback. We improve the readability of the two figures in Section 4.2.3, and please see Meta R1 for details. 
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have added relevant experiments conducted on different servers in Section 4.2.4 and 4.3.4. The results show that differences in server configurations have little impact on the ranking of the methods.
 Please refer to the response Meta R4 for more details.
%%%%%%%%%%%%%%%%%%%%%% W3 %%%%%%%%%%%%%%%%%%%%%%
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have added experiments on the Deep100M dataset in Section 4.2.4 and Section 4.3.4, and we observed some pitfalls. For more details, please refer to our response to Meta R7.
