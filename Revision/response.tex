...
\noindent
\textbf{Response:} Thanks for the suggestion! Ensuring the readability of the figures is indeed crucial. We modified both the figure and the related description in the paper to improve readability and ensure academic rigor. 

For performance under different attribute distributions(Figure 8), to illustrate how different algorithms are affected by varying attribute distributions, we have added robustness indicators (Strong and Weak) in the figure. This provides more intuitive visualization of how robust each algorithm performs across different attribute distributions. For performance under different attribute selectivity(Figure 10), to reflect the performance differences of each algorithm under varying attribute selectivities, we have added performance rankings for each algorithm across different selectivity levels in the figure. This provides a clearer illustration of how each algorithm is affected by attribute selectivity rates. We have revised the manuscript accordingly. 

\textcolor{blue}{"The proximity of the four curves (representing different attribute distributions) in the figure reflects the algorithm's sensitivity to distributional variations. When the curves converge closely, the algorithm demonstrates strong resilience against attribute distribution shifts. Conversely, greater divergence among the curves indicates higher distributional dependence. Accordingly, the "Strong" label signifies robust performance consistency across varying distributions, while "Weak" denotes pronounced performance fluctuations under distributional changes."} (Please see Section 4.2.3, highlighted in blue in the revised manuscript)
...
\noindent
\textbf{Response:} We believe this is a very good suggestion. Accordingly, we have added more complex multi-modal dataset experiments in the Multi-modal Queries sections of Section 4.2.4 and Section 4.3.4 in our revised paper. In the original version, we used the commonly adopted text2image dataset from existing multi-modal ANN algorithms, where the base vectors are images, but the queries are text. This setting naturally introduces a modality gap, and achieving good performance on this dataset usually requires algorithms to be more robust.

To further enrich the complexity of multi-modal queries, we additionally selected 500,000 text vectors and 500,000 image vectors to form a mixed base dataset, and we similarly selected 5,000 text vectors and 5,000 image vectors as query vectors. We then conducted both attribute filtering and range queries on this multi-modal dataset by combining text and image queries. This allows us to further analyze the robustness of the algorithms. The related experimental results are presented in the Multi-modal Queries sections of Section 4.2.4 and Section 4.3.4.

As for complex query types involving both attribute and range filters, to the best of our knowledge, there is currently no ANN algorithm (except for general-purpose systems like Milvus and Faiss) that can support both types of filtering simultaneously. For example, ACORN needs to build separate indices for each scenario and cannot support attribute and range filtering at the same time. Therefore, in our study, we chose to consider these query types separately, rather than jointly.
...
\textbf{YT-Audio-Real dataset:} For this dataset, we utilized the URLs provided by the YouTube8M dataset to crawl corresponding video category IDs and view counts as real attributes, and used view counts as the attribute for range filtering experiments.

\textbf{WIT-Real dataset:} We used the \texttt{clip-vit-base-patch32} model to encode the \texttt{page\_title} field from the WIT dataset into vectors, and extracted the language type and the length of the \texttt{context\_page\_description} field as real attributes, and used them for range filtering.
...
We conducted additional experiments on a server equipped with two Intel(R) Xeon(R) Gold 6330 CPUs @ 2.00GHz using the YT-Audio-Real dataset.
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We are deeply aware of this deficiency, so we added the search peak memory usage of each algorithm in \textcolor{blue}{Section 4.2.1} of attribute filtering and \textcolor{blue}{Section 4.3.1}  of range filtering, and carefully analyzed the reasons.

\textit{\textbf{ \textcolor{blue}{" Search peak memory.}}}

\textcolor{blue}{As shown in Figure, we can find that the peak memory usage of each algorithm during search does not differ significantly. Among them, CAPS and NHQ have relatively higher peak memory usage, while the database's peak memory usage is comparatively lower."}

\textit{\textbf{ \textcolor{blue}{"Search peak memory.}}}

\textcolor{blue}{As for the Search peak memory usage, we can find from the Figure that it is basically consistent with the Build peak memory usage trend. The algorithm needs to load the complete index into memory during search. Therefore, algorithms with high building overhead, such as WinFilter, also have high search overhead; algorithms with low building overhead, such as Faiss, also have low search overhead. For Milvus, search does not require full index loading and there is additional metadata overhead when building, so the performance is different from the trend."}
...
\noindent
\textbf{Response:} Thank you for your suggestion! In real-world applications, there may indeed exist more complex semantic correlations between vectors and their associated attributes or among different attributes themselves. To better reflect such scenarios, we introduced two real-world datasets, where both vectors and attributes are derived from actual data. For example, in the YouTube dataset, the attributes include video categories and view counts, while in the WIT dataset, the attributes correspond to the language of image descriptions.

We conducted experiments on both datasets and present the performance analysis for various algorithms in \textcolor{blue}{Sections 4.2.2 and 4.3.2}. Interestingly, our results indicate that using real attribute values has limited impact on the overall performance trends.
...
By incorporating the YT-Audio-Real and WIT-Real datasets with their respective real attributes, we believe our experimental study now better reflects real-world application scenarios, and, as you suggested, allows for a more comprehensive and meaningful evaluation of hybrid query functionality.
...
\noindent
\textbf{Response:} 
Thank you for your suggestion! We have carefully searched and found no papers describing the application of attribute distribution and attribute selectivity. However, the real-world application scenarios of attribute distribution and attribute selectivity are indeed very extensive. For attribute distribution, the label attributes of many recommended application scenarios in reality conform to the power-law distribution. For attribute selectivity: For example, if you need to query data in which an attribute accounts for a very small proportion of the total, choosing a more advantageous algorithm will greatly improve the search effect. There are also databases with actual applications that mention attribute selectivity, such as Milvus and AnalyticDB-V. For the two parts you suggested, we will add more comprehensive citations in the paper.
...
We have elaborated on the scalability implementations of various algorithms on the 100-million-scale Deep dataset in \textcolor{blue}{ Section XXX of our paper. We also provide a comprehensive analysis of their performance on large-scale datasets.}
