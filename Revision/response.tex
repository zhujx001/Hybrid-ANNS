...
\noindent
\textbf{Response:} 
Thank you for your suggestion. Using the ID as an attribute for range filtering is a prevalent method in current range filtering algorithm evaluations, which indeed has considerable limitations. To address this issue, we have changed two datasets (YT-Audio-Real, WIT-Real) to use real attributes. We observe that the impact of using real attribute versus using ID on the algorithms is negligible; the key factor remains the size of the query range, as shown in Figure~\ref{fig:attribute_distribution}. (Please see the third paragraph of Section 4.1.1, highlighted in blue in the revised manuscript.)
