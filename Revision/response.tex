...
\noindent
\textbf{Response:} 
Thank you for your suggestion. Using the ID as an attribute for range filtering is a prevalent method in current range filtering algorithm evaluations, which indeed has considerable limitations. To address this issue, we have changed two datasets (YT-Audio-Real, WIT-Real) to use real attributes. We observe that the impact of using real attribute versus using ID on the algorithms is negligible; the key factor remains the size of the query range, as shown in Figure~\ref{fig:attribute_distribution}. \textbf{(Please see the third paragraph of Section 4.1.1, highlighted in blue in the revised manuscript.)}
...
\noindent
\textbf{Response:} 
That is a great suggestion. In order to figure out if the server setting affects the relative ordering of methods, we conducted additional experiments on a server equipped with dual Intel(R) Xeon(R) Gold 6330 CPUs @ 2.00GHz featuring 128GB of RAM and 84 MiB of L3 cache using the YT-Audio-Real dataset.
...
We did observe slightly lower QPS on this server compared to our original one, which is mainly due to the the lower CPU frequency, reduced RAM capacity, and smaller L3 cache of the Xeon 6330 processors. \textbf{(Please see Section 4.2.4 and Section 4.3.4, highlighted in blue in the revised manuscript.)} 
...
\textcolor{blue}{"To investigate the impact of different server configurations on the performance of the algorithms, we conduct additional experiments on a server equipped with dual Intel(R) Xeon(R) Gold 6330 CPUs @ 2.00GHz featuring 128GB of RAM and 84 MiB of L3 cache using the YT-Audio-Real dataset, and the results are shown in Figure 12b. It is not difficult to see that the relative ranking of the algorithms under the two servers essentially remains unchanged, indicating that the performance of these methods is not significantly affected by differences in CPU configurations. Due to the lower CPU frequency, reduced RAM capacity, and smaller L3 cache, all methods demonstrate slightly lower QPS values."}
...
To address this crucial issue and enhance the real-world relevance of our evaluation, we have added two new real-world datasets with genuine attributes to our attribute filtering and range query experiments. For detailed information on these two new datasets and our improvements to range filtering, please see Meta-Review R3.

We have revised the manuscript accordingly.
%\textbf{ (Please see page 7, Section 4.2.1, and the "Single-Attribute Building and Single-Attribute Query" subsection of Section 4.2.2 on page 8, highlighted in blue in the revised manuscript.)} 
Furthermore, it should be noted that, in the "Single-Attribute Building and Single-Attribute Query" subsection of Section 4.2.2 on page 8, due to space constraints, we have removed two previous datasets (Audio and GloVe), as Audio is relatively small-scale, and GloVe's scale is similar to SIFT1M while its LID is similar to GIST1M. \textbf{(Please see Section 4.2.1 and the "Single-Attribute Building and Single-Attribute Query" part of Section 4.2.2, highlighted in blue in the revised manuscript.)}
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have already provided a detailed response to this issue in our reply to Meta-Review R5. Please refer to that response for more information.
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have made a detailed response to this issue in our reply to Meta-Review R6. Please refer to that response for more information.
...
Thank you for pointing this out. Reference [49] provides a comprehensive review and evaluation of graph-based approximate nearest neighbor (ANN) search algorithms, whereas our work is the first to establish a benchmark for hybrid queries. We originally cited [49] in the experimental setup section to adopt the evaluation metrics it proposed, as hybrid queries are inherently built upon ANN techniques. We acknowledge that the distinction between our work and the cited paper was not clearly explained in the earlier version. To address this, we have added a clarification in the Introduction and also referenced several experimental studies in the ANN literature. \textbf{(Please see Section 1.1, highlighted in blue in the revised manuscript.)}
...
\noindent
\textbf{Response:} 
Thank you for your suggestion. We have already addressed this issue in detail in our responses to Meta-Review R8 and R9. Specifically, we analyzed the strengths and weaknesses of each algorithm and recommended those with strong performance. Additionally, we provided a summary table illustrating how each algorithm performs across different application scenarios. For more details, please refer to our responses to Meta-Review R8 and R9.
...
To make the paper more readable, we have added a more detailed explanation of the range attribute in Section 4.3.2. \textbf{(Please see Section 4.3, highlighted in blue in the revised manuscript.)}
