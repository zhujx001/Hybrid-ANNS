...
# 定义数据路径
data_path = "/data/result/NHQ/result"
...
def get_all_result_files():
    return glob.glob(os.path.join(data_path, "*_results.csv"))
...
def extract_file_info(filename):
    ...
    if match:
        ...
        # 算法名称从目录结构中提取
        algorithm = os.path.basename(os.path.dirname(os.path.dirname(filename)))
...
def load_all_data():
    ...
    for file in files:
        dataset, queryset, algorithm = extract_file_info(file)
...
def plot_dataset_effect(all_data, output_dir):
    ...
    # 获取所有数据集
    datasets = set(k[0] for k in all_data.keys() if k[1] == query_set)
    ...
    plt.xlabel('Recall', fontsize=12)
    plt.ylabel('QPS', fontsize=12)
    plt.title(f"The impact of dataset on algorithm - query set {query_set}", fontsize=14)
    plt.grid(True, alpha=0.3)
    ...
    save_path = os.path.join(output_dir, f"all_datasets_all_algorithms.png")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for alg in algorithms:
        ...
        plt.xlabel('Recall', fontsize=12)
        plt.ylabel('QPS', fontsize=12)
        plt.title(f"{alg} - Comparison of performance on different datasets - query set {query_set}", fontsize=14)
        plt.grid(True, alpha=0.3)
        ...
        save_path = os.path.join(output_dir, f"{alg}_datasets_comparison.png")
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
# 7. 内存占用情况plot_dataset_effect
def main():
    # 创建输出目录
