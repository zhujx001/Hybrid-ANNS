...
def create_output_dirs():
    """创建输出目录"""
    dirs = ["plots/5_selectivity/mul_sel"]
    for d in dirs:
...
def plot_selectivity_experiments(all_data, output_dir):
    ...
    def x_transform(x):
        """自定义 x 轴变换: 让 0.0~0.6 变短，并让其长度等于 0.6~0.65 的两倍"""
        return np.where(x < 0.6, x / 6, (x - 0.6) * 2 + 0.1)

    def x_inverse_transform(x):
        """逆变换，恢复原始 x 值"""
        return np.where(x < 0.1, x * 6, (x - 0.1) / 2 + 0.6)
    ...
    for dataset in datasets:
        for thread_type in ["single_thread", "16_threads"]:
            ...
            for alg_idx, (algorithm, df_list) in enumerate(sorted(algorithm_data.items())):
                color = plt.cm.tab10(alg_idx % 10)  # 颜色映射
                linestyles = ['-', '--', '-.', ':']
                markers = ['o', 's', '^', 'd']
            ...
            plt.ylim(1e1, 1e6)  # 确保 y 轴从 10^1 到 10^6
            
            # 设置非线性 X 轴
            plt.gca().set_xscale(FuncScale(plt.gca(), (x_transform, x_inverse_transform)))

            # 设置 x 轴刻度
            # 设置 x 轴刻度，保留 0.0 但隐藏 0.0~0.6 之间的刻度
            xticks = np.append([0.0], np.arange(0.6, 1.05, 0.05))
            plt.xticks(xticks)
