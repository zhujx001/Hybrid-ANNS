...
%		\end{tabular}
	...
	\begin{tabular}{|l|l|*{12}{c|}}
		\hline
		\multicolumn{2}{|c|}{} & NHQ & Filtered & Stitched & CAPS & ACORN & UNG 
		& Puck & DSG & iRange & SeRF & UNIFY & Win \\ \hline
		...
		\multicolumn{2}{|l|}{RF}             & N & N & N & N & Y & N & N & Y & Y & Y & Y & Y \\ \hline
		\multicolumn{2}{|l|}{Dynamic Insert} & N & Y & N & Y & Y & Y & Y & N & Y & Y & Y & Y \\ \hline
		\multicolumn{2}{|l|}{Multi Thread}   & N & Y & Y & Y & Y & Y & Y & N & Y & Y & Y & Y \\ \hline
		\multicolumn{2}{|l|}{Base}           & Graph & Graph & Graph & IVF & Graph & Graph & IVF & Graph & Graph & Graph & Graph & Graph \\ \hline
...
%		
		...
		\footnotesize{
			\begin{minipage}{\linewidth}
				\vspace{0.1cm}
				\textcolor{orange}{Filtered: FilteredVamana
				Stitched: StitchedVamana
				Win: WinFilter}
...
%	\end{table}
	...
	We introduce 5 representative range filtering algorithms and present a comparative overview in Table~\ref{tab:compair_1}.
...
%	In addition, we evaluate the performance of different algorithms on the Out-of-Distribution (OOD) dataset Text2Image~\cite{texttoimage}.		
	...
	\begin{table}[t]
		...
		\resizebox{\columnwidth}{!}{
			\begin{tabular}{cccccc}
				...
				WIT-Real & 512 & 1,000,000 & 40,300 & \textcolor{blue}{xx}& Image \\
	...
	\begin{enumerate}
		...
		\item \textbf{Peak Memory Usage}: The maximum memory consumption observed during index construction \textcolor{orange}{and search}.
	\end{enumerate}
	...
	\textcolor{orange}{To explore the time and space overhead of different attribute filtering algorithms during index construction and search,}
	 we evaluate \textcolor{orange}{4} key metrics in the single-attribute scenario: index construction time, index size, \textcolor{orange}{build peak memory and search peak memory}. Notably, PASE on GIST1M and Enron is omitted, as it supports only data with dimensionality less than 512. 
	...
	NHQ adopts the kgraph approach for index construction, which requires loading the entire graph data into memory and maintaining a complete graph structure, leading to relatively high memory overhead. 
	...
	\sout{\textit{\textbf{Different Datasets.}}  }
...
%		\caption{Time and space overhead of index construction}
	...
	Similarly, we also evaluate \textcolor{orange}{4} key metrics mentioned above in \textit{Range Filtering} algorithms. \textcolor{orange}{Note: For the YT-Audio dataset with synthetic attributes and the YT-Audio dataset with real attributes, we found that the time and space overhead data of each algorithm are basically the same, which shows that the authenticity of the attribute has little effect on it.
	...
	ACORN-\(\gamma\) expands the neighbor list of each node to provide more candidate paths, which increases both computation and storage costs. It also prunes distant neighbors to save storage space. However, this introduces additional overhead and slows down the construction process.\textcolor{orange}{In contrast, as we have analyzed in the corresponding section on attribute filtering, ACORN-1 trades efficiency for accuracy, so the construction time is short. SeRF is built on HNSW, so the time is similar to and slightly lower than ACORN-1.}
