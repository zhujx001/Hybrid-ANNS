...
Faiss\footnote{\textcolor{blue}{https://github.com/facebookresearch/faiss}} \cite{Faiss} is an efficient similarity search library. It uses an ID selector to filter data before performing approximate nearest neighbor search. This supports hybrid search.In Faiss, we use the HQI batch optimization strategy in the IVF index. It groups queries with the same filter conditions. Each group only runs the filter once. It uses efficient matrix operations to compute vector similarity in batches.

PASE\footnote{\textcolor{blue}{https://github.com/alipay/PASE}} \cite{pase} is a vector index plugin for PostgreSQL. It uses a "search first, then filter" strategy. But because the candidate set size is fixed, it may not return enough top-k results.

VBASE\footnote{\textcolor{blue}{https://github.com/microsoft/MSVBASE}} \cite{vbase} is also a vector plugin for PostgreSQL. It applies filters during the HNSW index traversal. This ensures enough results.

Milvus\footnote{\textcolor{blue}{https://github.com/milvus-io/milvus}} \cite{milvus} is a vector database built for large-scale similarity search. It filters data first, then performs approximate nearest neighbor search. This supports hybrid search.
...
%}
	...
	\textcolor{violet}{To investigate the time and space overhead of different algorithms,} we evaluate \textcolor{orange}{4} key metrics in the single-attribute scenario: index construction time, index size, \textcolor{orange}{build peak memory and search peak memory}. Notably, PASE on GIST1M and Enron is omitted, as it supports only data with dimensionality less than 512. \textcolor{orange}{We use 64 threads to build the index for the large-scale Deep100M dataset, and 32 threads for the other six datasets. Notably, the VBASE and PASE algorithms only support single-threaded construction.}
...
\textcolor{orange}{
	As shown in Figure~\ref{fig:index_size_mb_comparison_query1}, PASE exhibits significantly larger index sizes across all supported datasets, mainly due to its reliance on a large amount of metadata. For regular-scale datasets, other methods show moderate variation. However, on large-scale dataset, Graph-based algorithms such as ACORN, Filtered-DiskANN, and UNG tend to exhibit a sharp increase in index size, primarily due to the large number of candidate neighbors they maintain. Overall, IVF-based methods tend to produce more compact indexes.
}
...
\begin{figure*}[t]
	...
	% 5. 全局总标题
	\caption{\textcolor{violet}Scalability Study (Range Filtering)}
	\label{fig:TrinityExtension——Attribution_main}
\end{figure*}
