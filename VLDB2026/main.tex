...
%	However, the DSG algorithm shows a noticeable performance gap between the Deep and YT-Audio datasets, despite their comparable LID. This indicates that DSG is less robust than other methods when facing dataset-dependent variations, possibly due to its sensitivity to data distribution beyond LID alone.
	...
	\begin{table}[t]
		...
		\resizebox{\columnwidth}{!}{
			\begin{tabular}{|l|c|c|}
				...
				\hline
				S8: OOD scenarios & CAPS &WinFilter  \\
				\hline
			\end{tabular}
		}
	...
	Faiss and CAPS can generate relatively compact indexes with low memory usage, which is suitable for resource-constrained situations (S6). Milvus and Faiss serve as general-purpose solutions (S5). UNG stands out for its high recall, precision, and QPS (S7). CAPS performs well in OOD scenarios (S8).
