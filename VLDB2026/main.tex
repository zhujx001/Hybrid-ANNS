...
%	\footnote{https://github.com/KGLab-HDU/TKDE-under-review-Native-Hybrid-Queries-via-ANNS}
	...
	\textcolor{violet}{In addition to the aforementioned datasets, we further evaluate the performance of various algorithms on the Out-of-Distribution (OOD) dataset Text2Image~\cite{texttoimage} and the large-scale dataset Deep100M to validate the algorithm generalization capability. Notably, the previously discussed datasets primarily relied on synthetic attributes for experimentation. Therefore, we additionally incorporated two datasets containing real attributes for a more comprehensive evaluation:}
	
	\textcolor{violet}{YT-Audio-Real: We utilized the URLs provided by the YouTube8M dataset to crawl the corresponding video category IDs and view counts as their real attributes.}

	\textcolor{violet}{WIT-Real: For this dataset, we encoded the page\_title field into vectors using the \texttt{clip-vit-base-patch32} model, and used the language type and the length of the context\_page\_description field as real attributes.}
%	In addition, we evaluate the performance of different algorithms on the Out-of-Distribution (OOD) dataset Text2Image~\cite{texttoimage}.		
	...
	As shown in Figure~\ref{fig:build_memory_mb_comparison}, on small-scale datasets, all methods except NHQ exhibit low memory usage. 
...
%		\caption{Time and space overhead of index construction}
	...
	%Unify exhibits the highest memory consumption, indicating that its index construction process is highly sensitive to the dimensionality of the data.
		...
		\textcolor{orange}{As shown in Figure~\ref{fig:rangeFilter_search_memory_mb}, Faiss and Milvus have the lowest peak memory usage during search. Faiss reason: Milvus reason:. WinFilter achieves the highest peak memory usage on almost all datasets. The reason is:. For the high-dimensional dataset WIT, UNIFY occupies the highest peak memory. The reason is:.}
		
		%Faiss和Milvus在搜索时的峰值内存最低，\textcolor{orange}{Faiss原因：Milvus原因：}。WinFilter在几乎所有数据集上取得最高的峰值内存，\textcolor{orange}{原因是：}。而对于高维数据集WIT，UNIFY占用最高的峰值内存，\textcolor{orange}{原因是：}。
