...
\begin{document}
	...
	\noindent\textbf{\underline{CAPS \cite{CAPS}.}} As graph-based methods, NHQ and Filtered-DiskANN often result in large index sizes. CAPS addresses this issue by being the first simultaneous filtering algorithm based on spatial partitioning, significantly reducing index size.
	...
	ACORN includes 2 index variants:  
	...
	\begin{table}[t]
		...
		\resizebox{\columnwidth}{!}{
			\begin{tabular}{cccccc}
				...
				Text2Image & 200 & 10,000,000 &10,000 & 59& Text2Image \\[2pt]
	...
	%serf: It incrementally builds the index in attribute order. It requires only a single pass and adds edges within local graphs. The structure is simple, with no redundant operations.
	
	\textit{\textbf{Index construction time.}}
	Figure~\ref{fig:rangeFilter_build_time} shows the single-threaded index construction time. Faiss builds the index fastest.
	...
	ACORN-\(\gamma\) expands each node’s neighbor list to offer more candidate paths, increasing computation and storage. It also prunes distant neighbors to reduce storage, but this adds extra overhead, slowing construction.

	\textit{\textbf{Index size.}} 
	As shown in Figure~\ref{fig:rangeFilter_index_size_mb}, Among all algorithms, Faiss has the smallest index structure because. On low-dimensional datasets (deep, yt-audio), PASE has the largest index size because its implementation may have wasteful alignment and padding of data structures, as well as the use of storage methods such as Base64 encoding, which significantly increase storage overhead.
	The index size of SeRF on low-dimensional datasets is not large, but on high-dimensional datasets (wit), SeRF is forced to store more edges and dynamic range information, so the index increases significantly
	
	\textit{\textbf{Peak memory usage.}}
	As shown in Figure~\ref{fig:rangeFilter_memory_mb}, For low-dimensional datasets (deep, yt-audio), Faiss takes up the lowest memory due to: . 
	DSG and WinFilter take up significantly more memory than other algorithms due to.
	
	For high-dimensional datasets WIT, iRange achieves the lowest peak memory usage. It pre-builds graphs only for a limited number of intervals, avoiding explicit index construction for all possible query ranges. Its index structure remains stable in high dimensions without significant graph growth. This effectively controls index size, making iRange more advantageous for high-dimensional datasets.
	In contrast, unify takes up the highest memory due to
