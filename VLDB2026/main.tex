...
%	DSG only supports single-threaded construction, resulting in the longest build time.}
	\textcolor{orange}{
	As shown in Figure~\ref{fig:rangeFilter_build_time}, Faiss demonstrates very fast index construction speed, primarily due to its strategy of sampling subsets and computing clustering centroids, which significantly reduces overall computational complexity. Both Milvus and Faiss adopt the IVF\_Flat architecture, achieving short construction times on both medium-scale and large-scale datasets. On large-scale datasets in particular, Milvus performs faster, mainly owing to its cache-aware optimization, which substantially reduces CPU cache misses.}
	...
	ACORN-\(\gamma\)  extends the neighbor list of each node to provide more candidate paths, which increases both computational and memory overhead. Although it prunes distant neighbors to save storage, the additional construction logic still incurs substantial time cost.}
		...
		\textcolor{orange}{
	DSG is an optimization over SeRF, incorporating a rectangular tree structure for index management. This makes the construction process more complex and costly. In addition, DSG only supports single-threaded construction, resulting in the longest construction time on large-scale datasets.}
