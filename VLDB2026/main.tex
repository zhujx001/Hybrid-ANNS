...
%	Unlike DSG, which relies on rectangle trees and range-aware edges, UNIFY adopts a segmentation-based approach and proposes a range query index structure. 
	...
	Many existing vector libraries, such as Faiss, and databases, such as Milvus, VBASE, and PASE, are designed specifically for ANN search, but they also support attribute filtering and range queries. We also experimented with them. However, since these libraries and databases do not rely on a single algorithm, their internal implementations are complex, using multiple methods and have detailed third-party documentation, so we will not elaborate on them here.
