...
%				\textit{Multi Thread} indicates whether the algorithm supports multi-threaded search.
	...
	Faiss offers an ID filtering mechanism, it operates solely during query execution and does not influence index building. ACORN-1 and ACORN-\(\gamma\) are specifically designed for single-attribute indexing. In database systems (VBASE, PASE, Milvus), \sout{when the system builds both B+ tree and vector indexes, queries typically utilize only the B+ tree.} 
	...
	To investigate this effect, we evaluate 4 AS levels (1\%, 25\%, 50\%, and 75\%) on the SIFT1M dataset. Using a single-thread execution environment, we compare the query performance of attribute filtering algorithms under varying AS conditions, as shown in Figure~\ref{fig:exp_5_1_1_SingleLabel_1thread}.
