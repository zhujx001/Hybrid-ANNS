...
\textcolor{orange}{For most algorithms, the query phase requires loading both the index and the dataset into memory, so its peak memory usage is usually comparable to that of the construction phase. However, as shown in the Figure~\ref{fig:search_memory_mb_comparison}, CAPS exhibits significantly higher peak memory during querying due to the need to dynamically maintain large candidate sets. In contrast, Milvus has lower query-time memory usage because it does not fully load the index and incurs additional metadata overhead during construction.
	...
	\begin{figure*}
		...
		\begin{center}
			\begin{minipage}{\linewidth}
				\centering	
				Note: \textit{Strong} represents robust to attribute distribution, \textit{Weak} represents sensitive to attribute distribution 
			\end{minipage}
		\end{center}
		}
	\end{figure*}
...
%	SeRF exhibits relatively weak overall performance, with a significant drop in recall under small-range query scenarios. This suggests that while its compressed graph structure is space-efficient, it has a substantial negative impact on query performance.
	...
	\textit{\textbf{\textcolor{blue}{Different Hardware Environments.}}}
	\textcolor{cyan}{As described in Section 4.2.4, we performed additional range filtering experiments using the YT-Audio-Real dataset on Server B, with results presented in Figure~\ref{fig:Different_Computer_range}. When compared with the experimental results of Server A shown in Figure~\ref{fig:exp_8_2}a, our findings remain consistent with those reported in Section 4.2.4. For brevity and due to space limitations, we omit further discussion of these redundant results.}
