...
%	\footnote{https://github.com/KGLab-HDU/TKDE-under-review-Native-Hybrid-Queries-via-ANNS}
	...
	For range filtering queries, \textcolor{red}{we evaluate 1 widely used real-world dataset: %Deep~\cite{yandex_deep_dataset}, YT-Audio~\cite{youtube8m_dataset}, and 
	WIT~\cite{wit_dataset}. Following prior work~\cite{DSG}, we use the data point ID (the primary key, ranging from $0$ to $n-1$) within each dataset as its attribute. This ensures that the data is initially sorted.}
